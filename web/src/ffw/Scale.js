...
FFW.Scale = Em.Object.create({
	...
	scale: function(){
		 ...
		 if(FFW.isAndroid){
			...
			/** Scale Application*/
			$('#wrapper').css({'-webkit-transform':'scale('+this.scalePoint+')','-webkit-transform-origin':'0px 0px' });
			...
			/** Align Application*/
			$('#wrapper').css({'margin-left': LeftShiftAppPercents + 'px', 'margin-top': UpShiftAppPercents + 'px'});
			...
			if(FFW.Backend.resizeVideo)
				FFW.Backend.sendFullScreenResponse($('#wrapper').offset().top,
													$('#wrapper').offset().left,
													this.scalePoint)
		 }
