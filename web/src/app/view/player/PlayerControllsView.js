...
MFT.PlayerControllsView = Em.ContainerView.create({
	...
	classNameBindings: ["isHidden:hidden", "MFT.VideoPlayerController.hideAnimation", "MFT.VideoPlayerController.disableControlls:hidden_display"],
	...
	stop: MFT.Button.extend({
		elementId: "stop",
		templateName: 'text',
		textBinding: 'MFT.locale.label.view_playercontrol_done',
		target: "MFT.VideoPlayerController",
		action: "stop",
		onDown: false
	}),
	...
	progressbar: Em.ContainerView.extend({
		...
		dragArea: Em.ContainerView.extend({
			...
			mark: Em.View.extend({
				...
				elementId: 'mark',
				classNameBindings: ['active:active','FFW.isAndroid::accelerate'],
				actionDown: function(){
					this.set('active', true);
					//activate drag in Android
					if (FFW.isAndroid && parseFloat(FFW.Backend.OSInfo.OSVersion) < 4){
						FFW.VideoPlayerRPC.sendDragState(true);
					}
