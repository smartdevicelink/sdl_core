...
public class ModuleTest {
    ...
    /**
     * Tag name for the {@link com.ford.syncV4.proxy.rpc.SyncMsgVersion} object
     */
    private final static String SYNC_MSG_VERSION_TAG_NAME = "syncMsgVersion";
    /**
     * Tag name for the {@link com.ford.syncV4.proxy.rpc.SyncMsgVersion#setMinorVersion(Integer)}
     * attribute
     */
    private final static String MINOR_VERSION_TAG_NAME = "minorVersion";
    /**
     * Tag name for the {@link com.ford.syncV4.proxy.rpc.SyncMsgVersion#setMajorVersion(Integer)}
     * attribute
     */
    private final static String MAJOR_VERSION_TAG_NAME = "majorVersion";
    /**
     * Tag name for Vector data type
     */
    private final static String VECTOR_TAG_NAME = "Vector";
    /**
     * Tag name for float numbers.
     */
    private static final String FLOAT_TAG_NAME = "Float";
    /**
     * Tag name for Boolean data type
     */
    private static final String BOOLEAN_TAG_NAME = "Boolean";
    /**
     * Tag name for String data type
     */
    private static final String STRING_TAG_NAME = "String";
    /**
     * Tag name for Integers
     */
    ...
    /**
     * Attribute name that defines AppId of the next test (in case of the next test is
     * {@link com.ford.syncV4.proxy.rpc.RegisterAppInterface})
     */
    private final static String NEXT_APP_ID_ATTR = "nextAppID";
    /**
     ...
     *
     * @param filePath path to XML test file or directory with XML test files to use.
     */
            ...
            while (eventType != XmlPullParser.END_DOCUMENT && !done) {
                ...
                switch (eventType) {
                    ...
                    case XmlPullParser.END_DOCUMENT:
                        ...
                        break;
                    case XmlPullParser.START_TAG:
                        if (tempName.equalsIgnoreCase(VECTOR_TAG_NAME)) {
                            ...
                            while (eventType != XmlPullParser.START_TAG && !nestedWhileDone) {
                                if (eventType == XmlPullParser.END_TAG) {
                                    if (parser.getName().equalsIgnoreCase(VECTOR_TAG_NAME)) {
                                        Logger.e("TESTING", "In " + VECTOR_TAG_NAME + " Loop, nestedWhileDone == true, END_TAG, name: " + name);
                            ...
                            while (eventType != XmlPullParser.END_DOCUMENT && !nestedWhileDone) {
                                tempName = parser.getName();
                                logParserDebugInfo("In " + VECTOR_TAG_NAME + " Loop, tempName: " + tempName);

                                switch (eventType) {
                                    case XmlPullParser.START_DOCUMENT:
                                        logParserDebugInfo("In " + VECTOR_TAG_NAME + " Loop, START_DOCUMENT, name: " + name);
                                        break;
                                    case XmlPullParser.END_DOCUMENT:
                                        logParserDebugInfo("In " + VECTOR_TAG_NAME + " Loop, END_DOCUMENT, name: " + name);
                                        break;
                                    case XmlPullParser.START_TAG:
                                        ...
                                        break;
                                    case XmlPullParser.END_TAG:
                                        logParserDebugInfo("In " + VECTOR_TAG_NAME + " Loop, END_TAG, name: " + name);
                                        if (tempName.equalsIgnoreCase(VECTOR_TAG_NAME)) {
                                            logParserDebugInfo("In " + VECTOR_TAG_NAME + " Loop, nestedWhileDone == true, END_TAG, name: " + name);
                                            nestedWhileDone = true;
                                        }
                                        break;
                                    case XmlPullParser.TEXT:
                                        ...
                                        break;
                                    case XmlPullParser.CDSECT:
                                        logParserDebugInfo("In " + VECTOR_TAG_NAME + " Loop, CDSECT, name: " + name);
                                        break;
                                    case XmlPullParser.ENTITY_REF:
                                        logParserDebugInfo("In " + VECTOR_TAG_NAME + " Loop, ENTITY_REF, name: " + name);
                                        break;
                                    case XmlPullParser.IGNORABLE_WHITESPACE:
                                        logParserDebugInfo("In " + VECTOR_TAG_NAME + " Loop, IGNORABLE_WHITESPACE, name: " + name);
                                        break;
                                    case XmlPullParser.PROCESSING_INSTRUCTION:
                                        logParserDebugInfo("In " + VECTOR_TAG_NAME + " Loop, PROCESSING_INSTRUCTION, name: " + name);
                                        break;
                                    case XmlPullParser.COMMENT:
                                        logParserDebugInfo("In " + VECTOR_TAG_NAME + " Loop, COMMENT, name: " + name);
                                        break;
                                    case XmlPullParser.DOCDECL:
                                        logParserDebugInfo("In " + VECTOR_TAG_NAME + " Loop, DOCDECL, name: " + name);
                                        break;
                                    default:
                                        break;
                                }
                            ...
                            logParserDebugInfo("out of " + VECTOR_TAG_NAME + " loop");
                        ...
                        } else if (tempName.equalsIgnoreCase(SYNC_MSG_VERSION_TAG_NAME)) {
                            logParserDebugInfo("In " + SYNC_MSG_VERSION_TAG_NAME);
                            SyncMsgVersion syncMsgVersion = new SyncMsgVersion();
                            Boolean nestedWhileDone = false;
                            eventType = parser.next();
                            while (eventType != XmlPullParser.END_DOCUMENT && !nestedWhileDone) {
                                tempName = parser.getName();
                                logParserDebugInfo("In " + SYNC_MSG_VERSION_TAG_NAME + " Loop, tempName: " + tempName);
                                switch (eventType) {
                                    case XmlPullParser.START_DOCUMENT:
                                        logParserDebugInfo("In " + SYNC_MSG_VERSION_TAG_NAME + " Loop, START_DOCUMENT, name: " + name);
                                        break;
                                    /*case XmlPullParser.END_DOCUMENT:
                                        logParserDebugInfo("In " + SYNC_MSG_VERSION_TAG_NAME + " Loop, END_DOCUMENT, name: " + name);
                                        break;*/
                                    case XmlPullParser.START_TAG:
                                        if (tempName.equalsIgnoreCase(INTEGER_TAG_NAME)) {
                                        ...
                                        break;
                                    case XmlPullParser.END_TAG:
                                        logParserDebugInfo("In " + SYNC_MSG_VERSION_TAG_NAME + " Loop, END_TAG, name: " + name);
                                        if (tempName.equalsIgnoreCase(SYNC_MSG_VERSION_TAG_NAME)) {
                                            logParserDebugInfo("In " + SYNC_MSG_VERSION_TAG_NAME + " Loop, nestedWhileDone == true, END_TAG, name: " + name);
                                            nestedWhileDone = true;
                                        }
                                        break;
                                    default:
                            ...
                            logParserDebugInfo("out of " + SYNC_MSG_VERSION_TAG_NAME + " loop");
                            hash.put(SYNC_MSG_VERSION_TAG_NAME, syncMsgVersion);
                        } else {
    ...
    private TestResult xmlTest() {
        ...
        //Logger.d("XML Test begin");

        if (mProxyService.getRestoreConnectionToRPCService().getHoldCount() > 0) {
            mProxyService.getRestoreConnectionToRPCService().acquireLock();
        }

        //Logger.d("XML Test init");
        ...
        Thread newThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < currentTest.getRequestsSize(); i++) {
                    ...
                    mCurrentTestIndex = i;
                    wrapper = currentTest.getRequests().get(mCurrentTestIndex);
                    /*if (!mProxyService.isSyncProxyConnected()) {
