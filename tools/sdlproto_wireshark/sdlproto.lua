...
function p_sdlproto.dissector(buf, pkt, root)
  ...
  local l_byte2 = buf(2, 1)
  if f_frameTypeValue_ControlFrame == l_frameType then
    subtree:add(f_frameInfo_ControlFrame, l_byte2)
  elseif f_frameTypeValue_ConsecutiveFrame == l_frameType then
    subtree:add(f_frameInfo_ConsecutiveFrame, l_byte2)
  else
    subtree:add(f_frameInfo, l_byte2):append_text(" (Reserved, should be 0x00)")
  end
