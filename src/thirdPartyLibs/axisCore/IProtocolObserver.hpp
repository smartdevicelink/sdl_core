...
public:
   /**
    * Callback informs client about session start if the session started from the other side.
    */
   virtual void sessionStartedCallback(const UInt8 sessionID) = 0;

   /**
    * Callback informs client about session end if the session ended from the other side.
    */
    ...
    */
   virtual void dataReceivedCallback(const UInt8 sessionID, const UInt32 dataSize) = 0;
