...
Json::Value GetCapabilitiesResponseMarshaller::toJSON(const GetCapabilitiesResponse& e)
{
  ...
  json["result"]["_Method"]=Json::Value("TTS.GetCapabilitiesResponse");

  {
    ...
    j.resize(i);
    while(i--)
      j[i]=SpeechCapabilitiesMarshaller::toJSON(e.capabilities[i]);
...
bool GetCapabilitiesResponseMarshaller::fromJSON(const Json::Value& json,GetCapabilitiesResponse& c)
{
  try
  {
    ...
    if(js["_Method"].asString().compare("TTS.GetCapabilitiesResponse")) return false;
    ...
    if(!js.isMember("capabilities") || !js["capabilities"].isArray())  return false;
    {
      ...
      if(i<1)  return false;
      if(i>100)  return false;
      std::vector<SpeechCapabilities> z(i);
      while(i--)
        if(!SpeechCapabilitiesMarshaller::fromJSON(js["capabilities"][i],c.capabilities[i]))  return false;
      c.capabilities=z;
    }
