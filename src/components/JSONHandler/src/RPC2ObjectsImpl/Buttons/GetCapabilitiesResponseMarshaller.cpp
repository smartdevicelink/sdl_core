...
Json::Value GetCapabilitiesResponseMarshaller::toJSON(const GetCapabilitiesResponse& e)
{
  ...
  json["result"]["_Method"]=Json::Value("Buttons.GetCapabilitiesResponse");

  {
    ...
    j.resize(i);
    while(i--)
      j[i]=ButtonCapabilitiesMarshaller::toJSON(e.capabilities[i]);
...
bool GetCapabilitiesResponseMarshaller::fromJSON(const Json::Value& json,GetCapabilitiesResponse& c)
{
  try
  {
    ...
    if(js["_Method"].asString().compare("Buttons.GetCapabilitiesResponse")) return false;
    ...
    if(!js.isMember("capabilities") || !js["capabilities"].isArray())  return false;
    {
      ...
      if(i<1)  return false;
      if(i>100)  return false;
      std::vector<ButtonCapabilities> z(i);
      while(i--)
        if(!ButtonCapabilitiesMarshaller::fromJSON(js["capabilities"][i],c.capabilities[i]))  return false;
      c.capabilities=z;
    }
