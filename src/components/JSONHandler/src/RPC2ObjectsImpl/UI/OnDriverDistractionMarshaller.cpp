#include "../include/JSONHandler/RPC2Objects/UI/OnDriverDistraction.h"
#include "../src/ALRPCObjectsImpl/DriverDistractionStateMarshaller.h"
#include "../src/ALRPCObjectsImpl/ResultMarshaller.h"
#include "OnDriverDistractionMarshaller.h"

using namespace RPC2Communication;
using namespace RPC2Communication::UI;
using namespace AppLinkRPC;

bool OnDriverDistractionMarshaller::checkIntegrity(OnDriverDistraction& e)
{
    return true;
}

bool OnDriverDistractionMarshaller::checkIntegrityConst(const OnDriverDistraction& e)
{
    return true;
}

bool OnDriverDistractionMarshaller::fromString(const std::string& s,OnDriverDistraction& e)
{
  try
  {
    Json::Reader reader;
    Json::Value json;
    if(!reader.parse(s,json,false))  return false;
    if(!fromJSON(json,e))  return false;
  }
  catch(...)
  {
    return false;
  }
  return true;
}

const std::string OnDriverDistractionMarshaller::toString(const OnDriverDistraction& e)
{
  Json::FastWriter writer;
  return checkIntegrityConst(e) ? writer.write(toJSON(e)) : "";
}

bool OnDriverDistractionMarshaller::fromJSON(const Json::Value& json,OnDriverDistraction& c)
{
  try
  {
    if(!json.isObject())  return false;
    if(!json.isMember("jsonrpc") || !json["jsonrpc"].isString() || json["jsonrpc"].asString().compare("2.0"))  return false;
    if(!json.isMember("method") || !json["method"].isString() || json["method"].asString().compare("UI.OnDriverDistraction"))  return false;
    if(!json.isMember("params")) return false;

    Json::Value js=json["params"];
    if(!js.isObject())  return false;

    if(!js.isMember("state") || !DriverDistractionStateMarshaller::fromJSON(js["state"],c.state))  return false;

  }
  catch(...)
  {
    return false;
  }
  return checkIntegrity(c);
}

Json::Value OnDriverDistractionMarshaller::toJSON(const OnDriverDistraction& e)
{
  Json::Value json(Json::objectValue);
  if(!checkIntegrityConst(e))
    return Json::Value(Json::nullValue);

  json["jsonrpc"]=Json::Value("2.0");
  json["method"]=Json::Value("UI.OnDriverDistraction");
  json["params"]=Json::Value(Json::objectValue);

  json["params"]["state"]=DriverDistractionStateMarshaller::toJSON(e.state);
  return json;

}
