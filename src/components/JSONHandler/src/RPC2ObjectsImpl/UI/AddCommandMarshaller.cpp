...
bool AddCommandMarshaller::checkIntegrityConst(const AddCommand& s)
{
  ...
  if(!MenuParamsMarshaller::checkIntegrityConst(s.menuParams))  return false;
...
Json::Value AddCommandMarshaller::toJSON(const AddCommand& e)
{
  ...
  json["params"]["menuParams"]=MenuParamsMarshaller::toJSON(e.menuParams);;
  return json;
}
...
bool AddCommandMarshaller::fromJSON(const Json::Value& json,AddCommand& c)
{
  try
  {
    ...
    if(!js.isMember("menuParams") || !MenuParamsMarshaller::fromJSON(js["menuParams"],c.menuParams))  return false;
