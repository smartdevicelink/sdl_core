...
bool AlertMarshaller::fromJSON(const Json::Value& json,Alert& c)
{
  try
  {
    ...
    if(js.isMember("AlertText1"))
    {
      ...
      c.AlertText1[0]=js["AlertText1"].asString();      if(c.AlertText1[0].length()>500)  return false;
    ...
    if(js.isMember("AlertText2"))
    {
      ...
      c.AlertText2[0]=js["AlertText2"].asString();      if(c.AlertText2[0].length()>500)  return false;
    ...
    if(js.isMember("duration"))
    {
      ...
      c.duration[0]=js["duration"].asInt();      if(c.duration[0]<3000 || c.duration[0]>10000)  return false;
