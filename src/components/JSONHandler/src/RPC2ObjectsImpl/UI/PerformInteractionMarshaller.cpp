...
#include "PerformInteractionMarshaller.h"
...
Json::Value PerformInteractionMarshaller::toJSON(const PerformInteraction& e)
{
  ...
  json["params"]["initialText"]=Json::Value(e.initialText);;
  {
    ...
    j.resize(i);
    while(i--)
      j[i]=TTSChunkMarshaller::toJSON(e.initialPrompt[i]);

    json["params"]["initialPrompt"]=j;
  }
  json["params"]["interactionMode"]=InteractionModeMarshaller::toJSON(e.interactionMode);;
  {
...
bool PerformInteractionMarshaller::fromJSON(const Json::Value& json,PerformInteraction& c)
{
  try
  {
    ...
    if(!js.isMember("initialPrompt") || !js["initialPrompt"].isArray())  return false;
    {
      ...
      while(i--)
        if(!TTSChunkMarshaller::fromJSON(js["initialPrompt"][i],c.initialPrompt[i]))  return false;
      c.initialPrompt=z;
    }

    if(!js.isMember("interactionMode") || !InteractionModeMarshaller::fromJSON(js["interactionMode"],c.interactionMode))  return false;
    ...
    if(js.isMember("timeout"))
    {
      ...
      c.timeout[0]=js["timeout"].asInt();      if(c.timeout[0]<5000 || c.timeout[0]>100000)  return false;
