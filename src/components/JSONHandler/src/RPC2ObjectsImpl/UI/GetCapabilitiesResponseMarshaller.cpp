...
{
  if(!DisplayCapabilitiesMarshaller::checkIntegrityConst(s.displayCapabilities))  return false;
...
Json::Value GetCapabilitiesResponseMarshaller::toJSON(const GetCapabilitiesResponse& e)
{
  ...
  json["result"]["displayCapabilities"]=DisplayCapabilitiesMarshaller::toJSON(e.displayCapabilities);;
  {
    ...
    j.resize(i);
    while(i--)
      j[i]=HmiZoneCapabilitiesMarshaller::toJSON(e.hmiZoneCapabilities[i]);
...
bool GetCapabilitiesResponseMarshaller::fromJSON(const Json::Value& json,GetCapabilitiesResponse& c)
{
  try
  {
    ...
    if(!js.isMember("hmiZoneCapabilities") || !js["hmiZoneCapabilities"].isArray())  return false;
    {
      ...
      if(i<1)  return false;
      if(i>100)  return false;
      std::vector<HmiZoneCapabilities> z(i);
      while(i--)
        if(!HmiZoneCapabilitiesMarshaller::fromJSON(js["hmiZoneCapabilities"][i],c.hmiZoneCapabilities[i]))  return false;
