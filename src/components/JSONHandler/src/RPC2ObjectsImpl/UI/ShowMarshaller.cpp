...
bool ShowMarshaller::checkIntegrityConst(const Show& s)
{
  ...
  if(s.alignment && (!TextAlignmentMarshaller::checkIntegrityConst(s.alignment[0])))  return false;
...
Json::Value ShowMarshaller::toJSON(const Show& e)
{
  ...
  if(e.alignment)
    json["params"]["alignment"]=TextAlignmentMarshaller::toJSON(e.alignment[0]);;
  if(e.statusBar)
...
bool ShowMarshaller::fromJSON(const Json::Value& json,Show& c)
{
  try
  {
    ...
    if(js.isMember("mainField1"))
    {
      ...
      c.mainField1[0]=js["mainField1"].asString();      if(c.mainField1[0].length()>500)  return false;
    ...
    if(js.isMember("mainField2"))
    {
      ...
      c.mainField2[0]=js["mainField2"].asString();      if(c.mainField2[0].length()>500)  return false;
    ...
    if(js.isMember("alignment"))
    {
      ...
      if(!TextAlignmentMarshaller::fromJSON(js["alignment"],c.alignment[0]))  return false;
    }
    ...
    if(js.isMember("statusBar"))
    {
      ...
      c.statusBar[0]=js["statusBar"].asString();      if(c.statusBar[0].length()>500)  return false;
    ...
    if(js.isMember("mediaClock"))
    {
      ...
      c.mediaClock[0]=js["mediaClock"].asString();      if(c.mediaClock[0].length()>500)  return false;
    ...
    if(js.isMember("mediaTrack"))
    {
      ...
      c.mediaTrack[0]=js["mediaTrack"].asString();      if(c.mediaTrack[0].length()>500)  return false;
