...
#include "OnAppUnregisteredMarshaller.h"
...
bool OnAppUnregisteredMarshaller::checkIntegrityConst(const OnAppUnregistered& s)
{
  ...
  if(s.reason && (!AppInterfaceUnregisteredReasonMarshaller::checkIntegrityConst(s.reason[0])))  return false;
...
Json::Value OnAppUnregisteredMarshaller::toJSON(const OnAppUnregistered& e)
{
  ...
  if(e.reason)
    json["params"]["reason"]=AppInterfaceUnregisteredReasonMarshaller::toJSON(e.reason[0]);;
  return json;
}
...
bool OnAppUnregisteredMarshaller::fromJSON(const Json::Value& json,OnAppUnregistered& c)
{
  try
  {
    ...
    {
      c.reason=new AppInterfaceUnregisteredReason();
      if(!AppInterfaceUnregisteredReasonMarshaller::fromJSON(js["reason"],c.reason[0]))  return false;
    }
