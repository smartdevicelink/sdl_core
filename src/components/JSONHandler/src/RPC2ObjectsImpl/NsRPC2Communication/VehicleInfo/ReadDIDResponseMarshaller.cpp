...
#include "../src/ALRPCObjectsImpl/V1/VehicleDataResultCodeMarshaller.h"
...
Json::Value ReadDIDResponseMarshaller::toJSON(const ReadDIDResponse& e)
{
  ...
  if(e.dataResult)
  {
    ...
    j.resize(i);
    while(i--)
      j[i]=NsAppLinkRPC::VehicleDataResultCodeMarshaller::toJSON(e.dataResult[0][i]);
...
bool ReadDIDResponseMarshaller::fromJSON(const Json::Value& json,ReadDIDResponse& c)
{
  try
  {
    ...
    if(js.isMember("dataResult"))
    {
      ...
      c.dataResult=new std::vector<NsAppLinkRPC::VehicleDataResultCode>();
      c.dataResult->resize(js["dataResult"].size());

      while(i--)
        if(!NsAppLinkRPC::VehicleDataResultCodeMarshaller::fromJSON(js["dataResult"][i],c.dataResult[0][i]))  return false;
    }
