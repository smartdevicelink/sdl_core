...
Json::Value ShowConstantTBTMarshaller::toJSON(const ShowConstantTBT& e)
{
  ...
  json["method"]=Json::Value("Navigation.ShowConstantTBT");
...
bool ShowConstantTBTMarshaller::fromJSON(const Json::Value& json,ShowConstantTBT& c)
{
  try
  {
    ...
    if(!json.isMember("method") || !json["method"].isString() || json["method"].asString().compare("Navigation.ShowConstantTBT"))  return false;
