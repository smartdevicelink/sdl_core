...
#include "../src/ALRPCObjectsImpl/V1/TurnMarshaller.h"
...
Json::Value UpdateTurnListMarshaller::toJSON(const UpdateTurnList& e)
{
  ...
  json["method"]=Json::Value("Navigation.UpdateTurnList");
  ...
  json["params"]=Json::Value(Json::objectValue);
  {
    ...
    j.resize(i);
    while(i--)
      j[i]=NsAppLinkRPC::TurnMarshaller::toJSON(e.turnList[i]);
...
bool UpdateTurnListMarshaller::fromJSON(const Json::Value& json,UpdateTurnList& c)
{
  try
  {
    ...
    if(!json.isMember("method") || !json["method"].isString() || json["method"].asString().compare("Navigation.UpdateTurnList"))  return false;
    ...
    if(!js.isMember("turnList") || !js["turnList"].isArray())  return false;
    {
      ...
      {
        NsAppLinkRPC::Turn t;
        if(!NsAppLinkRPC::TurnMarshaller::fromJSON(js["turnList"][i],t))
          return false;
         c.turnList[i]=t;
      }
    }
