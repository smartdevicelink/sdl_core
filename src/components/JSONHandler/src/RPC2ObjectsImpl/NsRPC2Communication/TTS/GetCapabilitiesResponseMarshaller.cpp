...
#include "../src/ALRPCObjectsImpl/V1/SpeechCapabilitiesMarshaller.h"
...
bool GetCapabilitiesResponseMarshaller::fromJSON(const Json::Value& json,GetCapabilitiesResponse& c)
{
  try
  {
    ...
    if(!js.isMember("capabilities") || !js["capabilities"].isArray())  return false;
    {
      ...
      while(i--)
      {
        NsAppLinkRPC::SpeechCapabilities t;
        if(!NsAppLinkRPC::SpeechCapabilitiesMarshaller::fromJSON(js["capabilities"][i],t))
          return false;
         c.capabilities[i]=t;
      }
    }
