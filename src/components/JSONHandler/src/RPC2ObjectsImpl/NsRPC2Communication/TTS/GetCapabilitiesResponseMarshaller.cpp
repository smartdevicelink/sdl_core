...
#include "../src/ALRPCObjectsImpl/V2/SpeechCapabilitiesMarshaller.h"
...
Json::Value GetCapabilitiesResponseMarshaller::toJSON(const GetCapabilitiesResponse& e)
{
...

  {
    ...
    j.resize(i);
    while(i--)
      j[i]=NsAppLinkRPCV2::SpeechCapabilitiesMarshaller::toJSON(e.capabilities[i]);
...
bool GetCapabilitiesResponseMarshaller::fromJSON(const Json::Value& json,GetCapabilitiesResponse& c)
{
  try
  {
    ...
    if(!js.isMember("capabilities") || !js["capabilities"].isArray())  return false;
    {
      ...
      while(i--)
      {
        NsAppLinkRPCV2::SpeechCapabilities t;
        if(!NsAppLinkRPCV2::SpeechCapabilitiesMarshaller::fromJSON(js["capabilities"][i],t))
          return false;
         c.capabilities[i]=t;
      }
    }
