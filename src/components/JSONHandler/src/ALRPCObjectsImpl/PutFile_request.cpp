...
PutFile_request::PutFile_request(const PutFile_request& c)
{
  *this=c;
}
...
PutFile_request::PutFile_request(void) : ALRPCRequest(PROTOCOL_VERSION,Marshaller::METHOD_PUTFILE_REQUEST),
      syncFileName(0),
    persistentFile(0)
{
}
...
void PutFile_request::reset_syncFileName(void)
{
  if(syncFileName)
    delete syncFileName;
  syncFileName=0;
