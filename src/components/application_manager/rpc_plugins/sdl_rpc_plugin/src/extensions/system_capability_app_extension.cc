...
bool SystemCapabilityAppExtension::subscribeTo(const SystemCapabilityType system_capability_type){
    LOG4CXX_DEBUG(logger_, system_capability_type);
    return subscribed_data_.insert(system_capability_type).second;
}

bool SystemCapabilityAppExtension::unsubscribeFrom(const SystemCapabilityType system_capability_type){
    LOG4CXX_DEBUG(logger_, system_capability_type);
...
bool SystemCapabilityAppExtension::isSubscribedTo(const SystemCapabilityType system_capability_type) const{
    LOG4CXX_DEBUG(logger_, system_capability_type);
    return subscribed_data_.find(system_capability_type) != subscribed_data_.end();
}   
...
void SystemCapabilityAppExtension::SaveResumptionData(ns_smart_device_link::ns_smart_objects::SmartObject&
    resumption_data){
    LOG4CXX_AUTO_TRACE(logger_);
...
void SystemCapabilityAppExtension::ProcessResumption(const smart_objects::SmartObject& resumption_data){
    LOG4CXX_AUTO_TRACE(logger_);
