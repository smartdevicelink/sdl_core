...
TEST(Settings, ReadParamStringWithSectionSuccess) {
  ...
  bool result = set.ReadParameter<std::string>("Remote Control",
                "InteriorVDCapabilitiesFile", &file_path);
  ASSERT_TRUE(result);
...
TEST(Settings, ReadParamStringWithSectionFail) {
  ...
  set.ChangeConfigFile("test.ini");
  std::string file_path;
  bool result = set.ReadParameter("Remote Control",
                "InteriorVDCFile", &file_path);
  ASSERT_FALSE(result);
  ASSERT_EQ("", file_path);
}
