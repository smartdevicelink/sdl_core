...
MFT.AudioPassThruPopUp = Em.ContainerView.create({
    ...
    activateBinding:    'MFT.SDLModel.AudioPassThruState',
    ...
    buttonRetry: MFT.Button.create({
        ...
        actionUp: function(){
            MFT.SDLController.callPerformAudioPassThruPopUpResponse( this );
        }
    }),
     ...
     * and show PopUp with data come from SDLCorel when activity flag become true
     */
    AudioPassThruActivity: function(){
        ...
        if( this.activate ){
            ...
            var self = this,
                data = MFT.SDLModel.AudioPassThruData;

            MFT.SDLModel.onPrompt(data.initialPrompt.ttsChunks);

            this.set('appName',    MFT.SDLController.getApplicationModel(data.appId).appName);
