...
FFW.Backend = FFW.RPCObserver.create({
	...
	/*
 	 ...
 	 */
	disconnect: function() {
		this.onAppStatusChangedUnsubscribeRequestId = this.client.generateId();
		var JSONMessage = {
			"jsonrpc":	"2.0",
			"id": 		this.onAppStatusChangedUnsubscribeRequestId,
			"method":	"MB.unsubscribeFrom",
			"params":	{
				"propertyName": "Backend.onAppStatusChanged"
			}
		};
	...
	onRPCRegistered: function () {
		...
		this.onAppStatusChangedSubscribeRequestId = this.client.generateId();
		var JSONMessage = {
			"jsonrpc":	"2.0",
			"id": 		this.onAppStatusChangedSubscribeRequestId,
	...
	checkSubscriptionResponse: function(response) {
		if (response.result != "OK") {
			Em.Logger.log("FFW.Backend: subscription response is not OK: " + response.result);
			return false;
		}
		return true;
	},
	...
	onRPCResult: function(response) {
		...
		} else if (response.id == this.getOSInfoRequestId)
		{
			...
			this.OSInfo.OSVersion 	= response.result.OSVersion;
	...
	onRPCNotification: function(notification) {
		...
		if (notification.method == "Backend.onAppStatusChanged") {
