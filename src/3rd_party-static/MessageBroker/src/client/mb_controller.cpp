...
void CMessageBrokerController::onMessageReceived(Json::Value message) {
  ...
  if (checkMessage(message, error)) {
    if (isNotification(message)) {
      ...
      if ("" != method) {
        if ("MB.registerComponent" ==
            method) {  // initialize mControllersIdStart
...
void CMessageBrokerController::sendJsonMessage(Json::Value& message) {
  ...
  if (!isNotification(message) &&
      !isResponse(message)) {  // not notification, not a response, store id and
                               // method name to recognize an answer
...
void CMessageBrokerController::prepareErrorMessage(int errCode,
...
std::string CMessageBrokerController::getDestinationComponentName(
    Json::Value& root) {
...
bool CMessageBrokerController::checkMessage(Json::Value& root,
                                            Json::Value& error) {
  ...
  try {
    ...
    DBG_MSG_ERROR(
        ("CMessageBrokerController::checkMessage() EXCEPTION has been "
         "caught!\n"));
    return false;
  }
}
