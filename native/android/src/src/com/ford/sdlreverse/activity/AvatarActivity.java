...
import com.ford.sdlreverse.downloader.DownloaderClient;
import com.ford.sdlreverse.downloader.XAPKFile;
import com.ford.sdlreverse.exception.ExtensionFileException;
import com.ford.sdlreverse.exception.MediaPlayerException;
import com.ford.sdlreverse.jsoncontroller.JSONBackendController;
import com.ford.sdlreverse.jsoncontroller.JSONPhoneController;
import com.ford.sdlreverse.jsoncontroller.JSONRateController;
import com.ford.sdlreverse.jsoncontroller.JSONVideoController;
import com.ford.sdlreverse.jsonserver.JSONServer;
import com.ford.sdlreverse.media.AvatarOnPreparedListener;
...
import com.ford.sdlreverse.util.ExtStorageUtils;
import com.ford.sdlreverse.util.MessageConst;
import com.ford.sdlreverse.util.Utils;
import com.ford.sdlreverse.util.WebViewUtils;
...
public class AvatarActivity extends Activity implements SurfaceHolder.Callback {
	...
	// for monkey testing
	// adb shell monkey -p com.ford.avatar -v 100
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		// MenuItem item = menu.findItem(R.id.mnuRate);
		//
		// if (ratePreferenceEnabled == null) {
		// SharedPreferences prefs = getSharedPreferences(
		// Const.SHPREF_APP_RATER, 0);
		// ratePreferenceEnabled = !prefs.getBoolean(
		// Const.SHPREF_RATER_NEVER_SHOW, false);
		// }
		// item.setEnabled(ratePreferenceEnabled);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		// final Intent settingsActivity = new Intent(getBaseContext(),
		// AcSettings.class);
		// startActivity(settingsActivity);
		// break;
		// case R.id.mnuRate:
		// mAppRater.showRateDialog(this, mRateController);
		// break;
		default:
			break;
		}
	...
	private JSONPhoneController mPhoneController;
	...
	private void makeCall(String number) {
		PhoneStateListener phoneListener = new PhoneStateListener() {
			private boolean bCallIsEnded = false;

			@Override
			public void onCallStateChanged(int state, String incomingNumber) {
				super.onCallStateChanged(state, incomingNumber);
				if (TelephonyManager.CALL_STATE_IDLE == state) {
					// when this state occurs and flag is set, restart app
					Log.i("PhoneStateListener", "CALL_STATE_IDLE");
					if (bCallIsEnded) {
						bCallIsEnded = false;
						mPhoneController.sendEndCallNotification(true);
					} else {
						bCallIsEnded = true;
					}
				} else if (TelephonyManager.CALL_STATE_OFFHOOK == state) {
					Log.i("PhoneProxy", "CALL_STATE_OFFHOOK");
				} else if (TelephonyManager.CALL_STATE_RINGING == state) {
					Log.i("PhoneProxy", "CALL_STATE_RINGING");
				}
			}
		};
	...
	private boolean removeOldVideoFiles() {
		...
		if (oldVideoFilesDirectory.exists()) {
			boolean success = true;
			for (File file : oldVideoFilesDirectory.listFiles()) {
				success &= file.delete();
			}
			return success;
		} else {
	...
	private void startRPCComponents() {
		...
		logMsg("create Phone controller");
		mPhoneController = new JSONPhoneController(this);
		mPhoneController.register(30);
		// wait for a while
		while (!mPhoneController.isRegistered()) {
			try {
				Thread.sleep(10);
			} catch (InterruptedException e) {
				e.printStackTrace();
	...
	/*
	 * Enum contains last active activity if lastActivity = MAIN_ACTIVITY - need
	 * start content verification
	 */
	private enum LastActivityEnum {
		SETTINGS_ACTIVITY, MAIN_ACTIVITY;
