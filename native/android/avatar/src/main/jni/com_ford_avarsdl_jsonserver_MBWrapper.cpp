...
#include "com_ford_avarsdl_jsonserver_MBWrapper.h"
...
void InvokeJSONServerSend(jint fd, jstring msg)
{
	...
	jclass clMBWrapper = penv->GetObjectClass(clObj);//penv->FindClass("com/ford/avarsdl/jsonserver/JSONServer");//penv->GetObjectClass(clObj);
	if(clMBWrapper == NULL)
	{
		return;
	}
